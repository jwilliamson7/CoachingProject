...
\abstract{Each NFL off-season brings a wave of head coaching changes, yet whether a given hire will end in dismissal remains poorly understood at the moment the hire is made. Prior work has largely predicted dismissal from results observed during a coach's tenure, a within-season view that leaves open how much is foreseeable on hiring day. We recast the problem as a firing-aware survival analysis. Across 371 head coaching stints in the modern era (1970--2025), we model the time from hire to involuntary non-retention (firing), treating voluntary departures (coaches who leave while still viable) as a competing risk and coaches still active at the data boundary as censored. Using only information available at the time of hire, we fit a cause-specific Cox proportional-hazards model as the primary estimator, benchmark it against parametric and gradient-boosted accelerated-failure-time models, and select features with a fully Cox-native stability-selection procedure. Dismissal proves only modestly foreseeable: the Cox model attains a Harrell concordance of 0.602 (95\% CI [0.560, 0.645]) and an integrated Brier score of 0.176 (95\% CI [0.160, 0.191]). What foreseeable signal exists is thin and largely structural. The single robust predictor is who is hired: internal promotions face a markedly higher firing hazard (HR 1.52, 95\% CI [1.15, 2.02]). Two further effects emerge in the multivariable model, with risk rising modestly with age at hire (HR 1.02 per year) and falling for coaches whose prior units were more efficient in yards per play (HR 0.74). Apart from that efficiency signal, the broad statistical record that hiring processes scrutinize forecasts dismissal, as a whole, little better than chance.}
...
Hiring a head coach is one of the most consequential personnel decisions an NFL franchise makes, and the record shows how often it is undone. Of the 371 stints we study in the modern era (1970 onward), 84\% ended in an involuntary parting, and the median coach survived only four seasons before being fired. Teams pour weeks of interviews, scheme presentations, and background checks into the choice, sometimes enlisting outside search consultants, yet reversals are common enough to raise a basic question: at the moment a coach is hired, before a single game has been played, how foreseeable is the eventual firing from the information the team actually has in hand?

Research on NFL coaching turnover consistently finds that dismissal is driven by a coach's performance in the job \citep{allen2012, salaga2020, foreman2019, farnell2021}. As we detail in Section~\ref{sec:litreview}, however, these studies share a structural approach: they explain dismissal using covariates realized \emph{during} the tenure, such as team-season win percentage, accumulated fines, or performance relative to expectations. The resulting models answer a retrospective question (given how a tenure unfolded, what is associated with firing) whose dominant answer, losing leads to firing, is of limited use to a team on hiring day.

A team on hiring day faces the harder, prospective version of this question: using only what can be observed when the contract is signed, how much of an eventual firing is already foreseeable? If pre-hire signal is strong, dismissals reflect foreseeable mismatches that sharper evaluation could reduce. If it is weak, the outcome is determined largely by events that unfold only after the contract is signed, and even a thorough hiring process is closer to an informed bet than its deliberateness suggests. The answer bears both on how franchises allocate their evaluation effort and on the broader debate over whether coaching changes accomplish what teams intend \citep{bryson2024}.

We frame NFL head coach dismissal as a pre-hire, firing-aware, competing-risks forecasting problem with calibrated and interpretable output, which separates the foreseeable component of the outcome from the larger part realized only after the hire. We then use that framing to ask how much of an eventual dismissal is written into the information available on hiring day, and where any foreseeable signal resides.

\section{Literature Review}
\label{sec:litreview}

Three strands of research inform this study: the determinants of NFL head coach dismissal, the use of survival analysis to model managerial tenure (and the reasons the Cox model has become its default tool), and the smaller body of work that forecasts sports outcomes from information fixed before the outcome is realized.

\subsection{Determinants of NFL head coach dismissal}

The economics of NFL coaching turnover has received sustained attention, and its consistent message is that dismissal is driven by performance. \citet{allen2012} estimate probit models of turnover over 1979--2006 and exploit the early-1990s introduction of the salary cap and free agency as a natural experiment, finding that both the probability of dismissal and the weight placed on talent and on-field results increased once the cap made measurable performance more decisive. \citet{salaga2020} analyze 208 head coach employment spells from 1985 to 2018 with survival methods and conclude that tenure and dismissal are largely a function of raw, and to a lesser degree relative, on-field performance accumulated during the tenure rather than anything known at the time of hire. \citet{foreman2019} fit a hazard model to 505 team-seasons (2000--2015) and show that player deviance, measured through fines, penalties, and off-field legal incidents, raises the dismissal hazard. \citet{farnell2021} examine the full coaching hierarchy and find that young, experienced, well-performing coordinators tend to be promoted, while older and poorly performing coaches tend to be fired, with race playing little role in either decision.

These studies share a structural approach: they explain dismissal using covariates realized \emph{during} the tenure, such as season win percentage, accumulated fines, or performance relative to expectations. The resulting models answer a retrospective question (given how a tenure unfolded, what is associated with firing), and their dominant answer, that losing leads to firing, is of limited use to a team on hiring day.

\subsection{Survival modeling of managerial tenure and the case for Cox}
\label{sec:litcox}

A coaching spell is naturally a time-to-event outcome: it has a start, an end that is either observed (a departure) or not yet observed (an ongoing tenure), and a duration whose distribution is the quantity of interest. Survival analysis is the standard framework for such data because it accommodates right-censoring, the coaches still employed at the end of the study window, without discarding those incomplete spells, which a simple regression on observed tenure length cannot do. The framework has been used across sports management. \citet{gilfix2020} study Major League Soccer manager longevity and find that performance relative to predecessors, league context, and age at hire all affect tenure duration. \citet{salaga2020} fit Cox survival models to 208 NFL coaching spells over 1985--2018, but, as in the dismissal studies above, their covariates are realized during the tenure (raw and relative on-field performance) rather than fixed at hire. Working outside the survival framing, \citet{cannon2025} isolate NBA head coaches' in-game scheme effects on win probability using Bayesian additive regression trees, quantifying coaching value rather than tenure.

Within survival analysis, the Cox proportional-hazards model \citep{cox1972} is among the most widely used time-to-event models across the empirical sciences, and we adopt it as our primary model for three reasons. It is semi-parametric, leaving the baseline hazard (the shape of firing risk over the course of a tenure) unspecified, which avoids committing in advance to a parametric clock for when coaches are fired; its coefficients exponentiate into interpretable hazard ratios, which suits a study whose substantive interest is in \emph{which} factors foreshadow firing; and it is the established choice in the cognate sports-management literature \citep{salaga2020, gilfix2020}, keeping our results comparable to prior work. Because an interpretable model is worth adopting only if it gives up no accuracy, Section~\ref{sec:models} benchmarks Cox against parametric accelerated-failure-time models and a gradient-boosted survival learner, and confirms that the proportional-hazards form predicts as well as the more flexible alternatives.
...
The analysis covers 371 permanent head coaching stints held by 264 distinct coaches who were hired between 1970 and 2025; 86 coaches account for more than one stint, and four (Bill Parcells, Pete Carroll, Chuck Knox, and Marty Schottenheimer) appear four times each. Because survival analysis treats an ongoing tenure as a right-censored observation rather than a missing outcome, we retain every coach still employed at the data boundary, including recent hires whose tenures have not yet concluded. Mid-season interim appointments that did not convert into a permanent role are excluded, and interim-to-permanent hires are anchored to the season in which the permanent tenure began. The duration of a stint is the number of seasons the coach held the position. As a check that dismissal is a meaningful outcome rather than noise, Appendix~\ref{app:winrate} relates tenure length to regular-season winning percentage, which rises with tenure.
...
The two quantities we estimate call for voluntary exits to be handled in two different ways, which is standard in competing-risks analysis \citep{austin2016}. For absolute risk, a voluntary departure is a competing event: such a stint can never subsequently end in a firing, so censoring it na\"ively would overstate the cumulative firing incidence (Section~\ref{sec:cif}). For the cause-specific hazard, which asks what raises the rate of firing among coaches still at risk of it, the correct handling is the reverse, with voluntary departures and active coaches both censored. We therefore apply each convention where it belongs, rather than choosing one for both purposes.

\subsection{Pre-hire features}

The feature pipeline produces a set of candidate predictors for each hire, all fixed at the time of hire and organized into eight families on two tiers: the coach being hired and the situation he inherits (Table~\ref{tab:feature_categories}). Several of these measure the same underlying construct more than once, so before selection and inference we reduce the candidates to a non-redundant working set of 74 features, by the procedure described in Section~\ref{sec:selection}. The description that follows, and the formal definitions in Appendix~\ref{app:features}, concern that retained set; Table~\ref{tab:feature_categories} reports both the candidate and the retained count for each family.

\begin{table}[!ht]
\caption{Pre-hire feature families on two tiers: attributes of the coach being hired, and attributes of the situation he inherits. ``Cand.'' is the number of candidate features the pipeline generates; ``Ret.'' is the number that survive the redundancy reduction of Section~\ref{sec:selection} and enter feature selection and inference. All values are measured at or before the hire date; Appendix~\ref{app:features} defines every retained feature.}
\label{tab:feature_categories}
\small
\begin{tabular}{lccl}
\toprule
Family & Cand. & Ret. & Description \\
\midrule
\multicolumn{4}{@{}l}{\emph{The coach (who is hired)}}\\
Core experience & 8 & 8 & Age at hire, prior HC stints, years by coaching level and role \\
Career structure & 17 & 16 & Employer history, role and side exposure, internal-hire indicator \\
Prior unit performance & 33 & 26 & Pooled, orientation-signed unit statistics from prior roles (App.~\ref{app:unitstats}) \\
Recent form & 6 & 5 & Percentile and trajectory of the coach's recent unit and team performance \\
\addlinespace
\multicolumn{4}{@{}l}{\emph{The situation (what is inherited)}}\\
Hiring-team results & 10 & 3 & Recent record, scoring, yards, turnovers, and playoff appearances \\
Inherited team quality & 6 & 4 & Team rating (SRS) components and schedule strength \\
Inherited roster & 12 & 10 & Roster and starter value, age, experience, retention, draft capital \\
Organizational instability & 2 & 2 & Predecessor tenure, distinct HCs in the prior decade \\
\midrule
& 94 & 74 & \\
\bottomrule
\end{tabular}
...
\label{eq:cox}
\end{equation}
leaving the baseline hazard $\lambda_0(t)$ unspecified. Our rationale for adopting Cox as the primary estimator, its semi-parametric flexibility, its interpretable hazard ratios, and its comparability with the prior literature, is set out in Section~\ref{sec:litcox}. For predictive evaluation the model is fit with elastic-net regularization (penalizer $0.5$, $\ell_1$ ratio $0.25$) to stabilize estimation in the presence of correlated features; for the reported inference (Section~\ref{sec:hazard}) we refit without penalization, as detailed there.
...
Several of the 94 candidate features measure the same construct more than once: the hiring team's pre-hire quality appears both as raw box-score aggregates and as the schedule-adjusted Simple Rating System, and the SRS total is by construction the sum of its offensive and defensive parts. Within each group of features whose pairwise correlation exceeds a high threshold we keep a single representative, preferring the schedule-adjusted measure over its raw counterpart and the SRS components over their total. This reduces the 94 candidates to 74 features for selection and inference; the block-level ablations of Section~\ref{sec:performance}, which characterize whole feature families rather than individual predictors, retain the full set.
...
\label{eq:stability}
\end{equation}
and we treat a feature as robustly selected when $\hat{\Pi}_k \geq \pi$ at threshold $\pi = 0.6$. So that the multivariable adjustment does not rest on a single covariate, and so that the next-strongest effects can be inspected rather than assumed away, we carry the five highest-frequency features into the reported hazard and prediction models, while treating the formal stable set as the one feature that clears the threshold; we report each feature's selection frequency alongside its estimate.

The pair $(q, \pi)$ is not arbitrary. For stability selection the expected number of falsely selected features is bounded by $\mathbb{E}[V] \leq q^2 / \{(2\pi - 1)\,p\}$ \citep{meinshausen2010}, with $p$ the candidate-pool size; the bound is finite only for $\pi > \tfrac{1}{2}$ and tightens as $q$ shrinks, which is why we set $\pi = 0.6$ rather than the looser one-half and keep $q$ small. Because $q$ is otherwise a modeling convention, we record the selection profile at $q \in \{10, 15, 20\}$ from the identical subsamples and report it in Section~\ref{sec:selection_results}, so the conclusions can be read against the choice. A paired corrected resampled $t$-test (Section~\ref{sec:eval}) detects no significant loss of concordance for this set relative to the larger set chosen by cross-validated concordance.
...
A survival model is assessed on two distinct properties. Discrimination is whether it ranks subjects in the correct order of risk; calibration is whether its predicted probabilities are numerically accurate. We report both.
...
Three choices govern the final inferential results. First, we fit the hazard-ratio model with cluster-robust (sandwich) standard errors \citep{lin1989} grouped on coach identity, because the same coach recurs across stints and ordinary standard errors would wrongly treat those stints as independent. Second, we test the proportional-hazards assumption, that each covariate acts with a hazard ratio which stays constant over the course of a tenure, using the Grambsch--Therneau test on scaled Schoenfeld residuals \citep{grambsch1994}; a small $p$-value would flag a hazard ratio that drifts over time. Third, we estimate absolute firing risk with the Aalen--Johansen cumulative incidence function \citep{aalen1978}, which gives the probability that a coach has been fired by a given season while accounting for the competing possibility of a voluntary departure.
...
Figure~\ref{fig:cif} shows the cumulative incidence of firing and of voluntary departure. Firing dominates throughout: by the end of the second season the cumulative firing incidence is 0.22 (95\% CI [0.18, 0.27]), against 0.01 ([0.00, 0.02]) for voluntary departure; by season four it is 0.55 ([0.50, 0.60]) against 0.03 ([0.01, 0.05]); and by season eight it is 0.81 ([0.76, 0.85]) against 0.06 ([0.04, 0.09]).

\begin{figure}[!ht]
\centering
\includegraphics[width=0.75\textwidth]{figures/survival_cif.png}
\caption{Aalen--Johansen cumulative incidence of firing (the event of interest) versus voluntary-while-viable departure (the competing event), with 95\% confidence bands. Firing incidence dominates at every horizon.}
\label{fig:cif}
\end{figure}
...
\midrule
Internal hire & 0.82 & 0.89 & 0.92 \\
Age at hire & 0.53 & 0.64 & 0.74 \\
Prior-unit yards-per-play efficiency & 0.44 & 0.59 & 0.71 \\
Years as NFL position coach & 0.48 & 0.60 & 0.68 \\
Inherited defensive SRS & 0.44 & 0.57 & 0.67 \\
\bottomrule
\end{tabular}
\end{table}
...
A concordance near 0.60 carries real but modest signal: presented with two coaches, the model orders their firing times correctly about three in five times. Calibration tells a complementary story. Figure~\ref{fig:calibration} shows the inverse-probability-weighted Brier score by horizon; the integrated Brier score is 0.176 (95\% CI [0.160, 0.191]), comfortably below the 0.25 of an uninformative model. The Brier score is the mean squared error of the predicted survival probabilities, so 0 marks perfect prediction and 0.25 a model that hands every coach the same coin-flip probability; at 0.176 the model's probabilities are informative but well short of certain, in keeping with the modest concordance. Prediction error is lowest in the first season and at long horizons, and peaks near 0.25 in the third and fourth seasons, the window in which firing decisions are most contested and least determined by pre-hire information.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\textwidth]{figures/survival_calibration.png}
\caption{Time-dependent inverse-probability-of-censoring-weighted Brier score for the Cox model by horizon, with the integrated Brier score in the title. The dashed line at 0.25 marks an uninformative model; error peaks in the contested third and fourth seasons.}
...
Table~\ref{tab:hazard} and Figure~\ref{fig:forest} report the cause-specific firing hazard ratios from a Cox model on the five highest-frequency features with cluster-robust standard errors on coach identity. We report this model \emph{without} penalization: with five near-uncorrelated features (the redundancy pruning of Section~\ref{sec:selection} having removed collinear duplicates, no pair correlates above 0.7) and 312 events, the unpenalized maximum-likelihood fit is stable, whereas a penalized fit would bias the coefficients toward a hazard ratio of one and would not support valid confidence intervals or $p$-values. The proportional-hazards assumption is not rejected for any covariate (Grambsch--Therneau minimum $p = 0.20$).
...
These findings have a practical bearing on how teams hire. Much of a coaching search is devoted to film study of a candidate's prior offenses and defenses and to parsing his statistical record, yet that record is, as a whole, among the information our models find least predictive of whether the hire will end in firing. This is not a case for ignoring the football record, which plausibly bears on performance once a coach is in place, nor against the modest protective signal in prior-unit efficiency; it is evidence that most of the record's predictive content has been competed away by the time a candidate reaches a head-coaching interview, when the league has already sorted candidates on observable competence and little residual variation in pre-hire credentials remains to separate those who will survive from those who will not. What stays foreseeable is largely structural, and the clearest structural factor, the elevated hazard attached to internal and interim-to-permanent promotions, may reflect the hiring situation as much as the coach: a franchise that promotes from within under duress may be acquiring the distress along with the candidate.

The modest ceiling also speaks to the debate over whether coaching changes accomplish what teams intend \citep{bryson2024}. If the eventual success of a hire is mostly unwritten on hiring day, the recurring cycle of firing and replacing looks less like the correction of identifiable mistakes than a sequence of draws from a distribution teams cannot fully see in advance. The practical implication is one of calibrated expectations: treat the hire as a genuine bet, price in the irreducible uncertainty our calibration results quantify, and be skeptical of after-the-fact narratives that recast a largely unforeseeable outcome as an avoidable error.
...
Framing NFL head coach dismissal as a firing-aware, competing-risks survival problem conditioned only on pre-hire information yields a clear and somewhat sobering picture. Dismissal is only modestly foreseeable at the moment of hiring, the proportional-hazards model is as accurate as more flexible alternatives, and what foreseeable signal exists lies chiefly in the structural circumstances of the hire, above all whether the coach is promoted from within, with weaker contributions from age and from the efficiency of the coach's prior units, rather than in the broad football record that hiring processes emphasize. The same pre-hire, firing-aware question extends naturally to other coaching markets with frequent turnover and a public hiring record, such as Major League Soccer and the NBA \citep{gilfix2020}; whether the modest ceiling we document is particular to the NFL or general to elite coaching labor markets is a question we leave to future work. Coaching changes may be frequent, but at the moment they are set in motion, their eventual unraveling is mostly still unwritten.
...
Table~\ref{tab:featuredefs} defines the 74 retained features (Table~\ref{tab:feature_categories}), grouped by family. The 26 retained pooled-unit statistics are a subset of the 33 listed in Appendix~\ref{app:unitstats}; the seven dropped as redundant duplicates are total yards, points scored, interceptions, net yards per attempt, rushing first downs, penalty yards, and points per drive, each closely tracked by a retained measure of the same construct. All values are measured at or before the hire date.

{\footnotesize
\begin{longtable}{@{}l p{8.6cm}@{}}
\caption{Definitions of the 74 retained pre-hire features by family. The pooled-unit family (26 statistics) is itemized separately in Appendix~\ref{app:unitstats}.}
...
\caption{Mean regular-season winning percentage by tenure length. Winning percentage rises monotonically with tenure (0.31, 0.41, and 0.54 for short, medium, and long stints; Kruskal--Wallis $p < 10^{-30}$), indicating that dismissal tracks on-field success and is a meaningful outcome to model.}
\label{fig:winrate}
\end{figure}
...
For predictive evaluation the Cox model uses elastic-net regularization (penalizer $0.5$, $\ell_1$ ratio $0.25$), tuned by coach-level cross-validated concordance and held fixed across the 50 resamples. The reported hazard-ratio model (Table~\ref{tab:hazard}) is unpenalized, for the inferential reasons given in Section~\ref{sec:hazard}. Stability selection first reduces the 94 candidate features to a 74-feature non-redundant pool by collapsing groups of highly correlated duplicates to a single representative (Section~\ref{sec:selection}), then draws 2000 coach-level subsamples at 50\% sampling and retains features with selection frequency at least 0.6 among the top 10 per subsample, with the selection profile also recorded at the top 15 and 20. The XGBoost-AFT benchmark uses a normal AFT loss and was tuned by random search over 200 draws from a grid spanning the number of boosting rounds, learning rate, tree depth (2 to 5), row and column subsampling, minimum child weight, and $\ell_2$ regularization, so that interaction structure up to depth five was available to it; the parametric AFT benchmarks use a small ridge penalty ($0.01$). All resampling uses an 80/20 coach-level train/test split.
