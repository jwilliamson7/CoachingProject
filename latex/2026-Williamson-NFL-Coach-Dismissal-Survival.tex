...
\abstract{Head coach turnover in the National Football League is frequent, and a coach's tenure is the clearest available measure of whether a hire succeeded. Yet whether a hire will end in firing is poorly understood when the coaching contract is signed. Prior work has largely modeled the in-season firing decision from in-tenure results, leaving open how much is foreseeable at the time of hire. We recast the problem as a firing-aware survival analysis. Across 371 head coaching stints in the modern era (1970--2025), we model the time from hire to firing, treating voluntary departures as a competing risk and active coaches as censored, from pre-hire information. A cause-specific Cox model, benchmarked against parametric and gradient-boosted alternatives and fit on Cox-native stability-selected features, proves modestly predictive, with a Harrell concordance of 0.602 (95\% CI [0.560, 0.645]) and an integrated Brier score of 0.176 (95\% CI [0.160, 0.191]). The one robust signal is structural: internal promotions face a higher firing hazard than external hires (HR 1.52, 95\% CI [1.15, 2.02]), with weaker effects from age at hire and prior-unit yards-per-play efficiency. The broad on-field record hiring searches examine, points, yards, and turnovers against league average, forecasts dismissal no better than chance.}
...
The framework has been used across sports management. \citet{gilfix2020} study Major League Soccer manager longevity and find that performance relative to predecessors, league context, and age at hire all affect tenure duration. \citet{salaga2020} fit Cox survival models to 208 NFL coaching spells over 1985--2018, but their covariates are realized during the tenure (raw and relative on-field performance) rather than fixed at hire.
...
The analysis covers 371 non-interim head coaching stints held by 264 distinct coaches who were hired between 1970 and 2025; 86 coaches account for more than one stint. Mid-season interim appointments that did not convert into a non-interim role are excluded, and interim appointments later retained on a non-interim basis are anchored to the season in which that tenure began. The duration of a stint is the number of seasons the coach held the position. As a check that dismissal is a meaningful outcome rather than noise, Appendix~\ref{app:winrate} relates tenure length to regular-season winning percentage, which rises across short, medium, and long stints (Kruskal--Wallis $p < 0.001$).
...
The analysis draws on 74 candidate pre-hire features for each hire, all fixed at the time of hire and organized into eight families on two tiers: the coach being hired and the situation he inherits (Table~\ref{tab:feature_categories}). From these candidates the model selects a smaller set by stability selection (Section~\ref{sec:selection}). The description that follows, and the formal definitions in Appendix~\ref{app:features}, concern this candidate set.

\begin{table}[!ht]
\caption{The 74 candidate pre-hire features on two tiers: attributes of the coach being hired, and attributes of the situation he inherits. ``\#'' is the number of features each family contributes to the candidate set; the prior-unit family is detailed in Appendix~\ref{app:unitstats}. All values are measured at or before the hire date; Appendix~\ref{app:features} defines every feature.}
...
Prior unit performance & 26 & Pooled, orientation-signed unit statistics from prior roles (App.~\ref{app:unitstats}) \\
...
A coach accumulates on-field statistics in several prior roles, as an offensive coordinator, a defensive coordinator, or a head coach, and each role speaks to a different side of the ball. The natural way to record this would be a separate statistical block for each role and side: an offensive block filled only by a coach's offensive-coordinator seasons, a defensive block filled only by his defensive-coordinator seasons, and so on. But almost no coach has held every role, so for any individual most of those blocks would stand empty, a career defensive coach having no offensive-coordinator statistics and a first-time coordinator no head-coaching ones. Rather than carry these mostly empty parallel blocks, we pool a coach's prior role-seasons into a single oriented \emph{unit} block of 33 statistics (itemized in Appendix~\ref{app:unitstats}). Each season the coach worked is mapped to the side of the ball that role controlled, and the contribution is signed so that larger values are always better: an offensive coordinator's season contributes its team's offensive output; a defensive coordinator's season contributes the suppression of the opposing offense; a head coach's season contributes both. Turnovers and penalties are sign-flipped so that fewer is better. Per-season values are normalized relative to that season's league distribution,
\begin{equation}
...
\label{eq:cox}
\end{equation}
leaving the baseline hazard $\lambda_0(t)$ unspecified. For predictive evaluation the model is fit with elastic-net regularization, whose penalty strength and $\ell_1$ mixing are chosen by coach-level cross-validated concordance over a grid (penalizer $0.5$ and $\ell_1$ ratio $0.25$ at the optimum; Appendix~\ref{app:hyperparameters}). For the reported inference (Section~\ref{sec:hazard}) we refit without penalization, as detailed there.
...
Table~\ref{tab:hyperparameters} reports the search spaces and selected values for the two tuned models. The Cox elastic-net penalty was chosen by exhaustive grid search maximizing coach-level cross-validated concordance; the XGBoost-AFT benchmark was tuned by random search over 200 draws from the grid shown, making interaction structure up to depth five available to it. Both configurations were held fixed across the 50 resamples. The reported hazard-ratio model (Table~\ref{tab:hazard}) is unpenalized, for the inferential reasons given in Section~\ref{sec:hazard}, and the parametric AFT benchmarks use a small fixed ridge penalty ($0.01$).

Stability selection operates on the 74 candidate features (Section~\ref{sec:selection}), drawing 2000 coach-level subsamples at 50\% sampling and retaining features with selection frequency at least 0.6 among the top 10 per subsample, with the selection profile also recorded at the top 15 and 20. All resampling uses an 80/20 coach-level train/test split.
