...
To our knowledge, no prior journal publications have attempted to predict the success of NFL coaching hires through statistical learning techniques. Currently, the NFL is only just beginning to implement artificial intelligence (AI) in play calling prediction \citep{datarobot2020}.
...
\begin{table}[!ht]
\caption{Feature categories and counts for the 150 engineered features used in coach tenure prediction. Each coaching hire is characterized by core experience metrics, performance statistics from prior coordinator and head coaching roles, and the hiring team's recent performance.}
...
\midrule
Model Improvement** & 5.3$\times$ \\
\bottomrule
\multicolumn{2}{l}{\footnotesize *Assuming all GMs believe their selected HC is Class 2} \\
\multicolumn{2}{l}{\footnotesize **Model F1 vs. human baseline} \\
\end{tabular}
\end{table}
...
Table~\ref{tab:tenure_predictions} shows the ordinal model's predictions for coach tenure for the 21 head coaches hired since 2022. This table also shows the probabilities associated with each class prediction; these probabilities sum to 1, and the class with the greatest probability is the final predicted class. As a reminder, Class 0 represents coaches who remain a head coach for 1--2 years, Class 1 represents coaches who remain a head coach for 3--4 years, and Class 2 represents coaches who remain a head coach for 5+ years.
...
While the ordinal classification model demonstrates strong predictive performance, understanding \textit{which} features drive predictions provides actionable insights for team decision-makers. We employ SHAP (SHapley Additive exPlanations) values \citep{lundberg2017unified} to quantify each feature's contribution to model predictions. For the Frank-Hall ordinal classifier, we compute SHAP values for each binary classifier and aggregate them using mean absolute values across classifiers.

\subsection{Offensive vs.\ Defensive Metrics}

We categorize the 150 features into six groups based on their source and type: Core Experience (8 features), OC Statistics (33 offensive features from coordinator tenure), DC Statistics (33 defensive features from coordinator tenure), HC Team Statistics (33 features reflecting team offensive performance during head coaching tenure), HC Opponent Statistics (33 features reflecting opponent performance, i.e., defensive effectiveness, during head coaching tenure), and Hiring Team Context (10 features). Table~\ref{tab:shap_categories} presents the mean absolute SHAP values by category.

\begin{table}[htbp]
\centering
\caption{Feature importance by category measured using mean absolute SHAP values. Total $|\text{SHAP}|$ is the sum across all features in each category; Avg $|\text{SHAP}|$ is the mean per feature. Higher values indicate greater contribution to tenure predictions.}
...
\subsection{Consistency Across Eras}

One concern is whether the defensive emphasis reflects historical biases in coaching evaluation that may have diminished in the modern passing-oriented NFL. We segment the data into three eras based on major NFL rule changes and league evolution: 1920--1969 ($n = 262$), 1970--1999 ($n = 210$), and 2000--2021 ($n = 163$). Table~\ref{tab:shap_era} presents the defensive-to-offensive importance ratio by era.

\begin{table}[htbp]
\centering
\caption{Defensive-to-offensive SHAP importance ratio by era. The ratio compares summed defensive feature importance (DC Stats + HC Opponent Stats) to offensive feature importance (OC Stats + HC Team Stats) for each coaching hire. Per-sample median is computed across all coaches within each era.}
\label{tab:shap_era}
\begin{tabular}{lrrr}
\toprule
Era & $n$ & Def/Off Ratio & Per-Sample Median \\
\midrule
1920--1969 & 262 & 1.69$\times$ & 1.68 \\
1970--1999 & 210 & 1.76$\times$ & 1.78 \\
2000--2021 & 163 & 1.61$\times$ & 1.68 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Coach Background Analysis}
...
Table~\ref{tab:shap_background} presents the results. Both offensive-background and defensive-background coaches show significantly higher importance for defensive metrics (Wilcoxon signed-rank $p < 0.0001$ for both groups). Crucially, the magnitude of defensive bias does not differ between groups (Mann-Whitney $p = 0.660$; bootstrap 95\% CI for difference: $[-0.22, 0.31]$).

\begin{table}[htbp]
\centering
\caption{Defensive-to-offensive SHAP importance ratio by coach background. Coaches are classified by their primary pre-head-coaching experience. Wilcoxon $p$ tests whether the median ratio exceeds 1.0; Mann-Whitney $p$ tests whether ratios differ between offensive and defensive background coaches.}
\label{tab:shap_background}
\begin{tabular}{lrrr}
\toprule
Coach Background & $n$ & Median Def/Off Ratio & Wilcoxon $p$ \\
\midrule
Offensive Background & 201 & 1.81$\times$ & $< 0.0001$ \\
Defensive Background & 159 & 1.77$\times$ & $< 0.0001$ \\
\midrule
\multicolumn{3}{l}{Mann-Whitney (between groups)} & $p = 0.660$ \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:top_features} presents the ten most predictive individual features. Notably, situational defensive metrics dominate: red zone attempts faced (as DC), fourth-down conversion rate allowed, and third-down metrics. These suggest that organizations, implicitly or explicitly, weight high-leverage defensive situations when evaluating head coaches.

\begin{table}[htbp]
\centering
\caption{Top 10 most predictive features ranked by mean absolute SHAP value across all 635 coaching hires. Higher values indicate greater contribution to tenure classification. DC = Defensive Coordinator tenure; HC = Head Coach tenure.}
...
Figure~\ref{fig:shap_categories} visualizes the category-level importance, and Figure~\ref{fig:shap_era} shows the consistency of defensive bias across eras.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\columnwidth]{figures/shap_offense_defense_split.png}
\caption{SHAP feature importance by category, split into offensive and defensive metrics. Defensive categories (blue) show higher average importance than offensive categories (orange).}
\label{fig:shap_categories}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\columnwidth]{figures/shap_by_era.png}
\caption{Distribution of per-coach defensive-to-offensive SHAP importance ratios by era. Box plots show median (center line), interquartile range (box), and 1.5$\times$IQR whiskers. The defensive bias is consistent across all eras (Kruskal-Wallis $p = 0.401$), indicating that defensive metrics have been persistently more predictive regardless of NFL rule changes or coaching philosophies.}
\label{fig:shap_era}
\end{figure}
